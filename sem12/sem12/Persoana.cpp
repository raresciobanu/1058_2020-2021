...
Persoana::Persoana(string nume, int varsta, const char* telefon, double salariu)
{
	this->nume = nume;
	this->varsta = varsta;
...
void Persoana::citireDinFisierBinar(ifstream& intrare)
{
	...
	intrare.read((char*)&salariu, sizeof(salariu));
...
double Persoana::calculeazaSalariuDupaMarireProcentuala(double procentaj)
{
	salariu += salariu * procentaj / 100;
	return salariu;
}
